...
### 📊 k6 부하 테스트 결과 (비관적 vs 낙관적 비교 분석)
| 지표 | 비관적 락 (Pessimistic) | 낙관적 락 (Optimistic) | 결과 요약 |
| :--- | :---: | :---: | :--- |
| **성공 건수** | 100건 (33%) | **100건 (33%)** | 데이터 정합성 완벽 유지 |
| **평균 응답 시간** | 102.62ms | **59.26ms** | **낙관적 락이 42% 빠름** |
| **최대 응답 시간** | 1,040ms | **498ms** | **낙관적 락이 2배 이상 쾌적** |

- **분석**: **비관적 락**은 데이터는 안전하게 지키지만, 줄을 세우는 방식 때문에 응답 시간이 길고 서버 가용성이 떨어집니다. 반면 **낙관적 락**은 자물쇠를 채우지 않아 매우 빠르지만, 충돌 시 즉시 실패하므로 별도의 재시도 처리가 필수적입니다.
- **정합성 검증 성공**: 총 300개의 동시 요청 중 `maxCapacity(100)`를 초과하는 요청은 모두 차단되고, 정확히 100건(33.3%)만 성공함을 확인하여 동시성 환경에서의 데이터 정합성을 완벽히 증명했습니다.
- **인프라 대조 실험**: **HikariCP의 Maximum Pool Size를 10에서 20으로 증설**하여 테스트를 반복했으나, 성능 지표에 유의미한 변화가 없음을 확인했습니다. 이는 현재 시스템의 병목이 '하드웨어/인프라 자원'이 아닌 **'데이터베이스 락 전략 및 소프트웨어 로직'**에 있음을 시사합니다.
- **결론**: 두 방식의 장점(빠른 속도 + 순차 처리)을 결합하고 인프라 효율성을 극대화하기 위해 **V2 로드맵(Redis 분산 락, 비동기 대기열)**으로의 전환을 확정했습니다.
